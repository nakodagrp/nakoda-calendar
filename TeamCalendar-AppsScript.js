// ════��═══════════════════════════���══════════════════════════════
// TEAM CALENDAR & TASK MANAGER — Google Apps Script Backend
// ═══════════��═══════════════════════════════════��═══════════════
// SETUP STEPS:
// 1. Create a Google Sheet and copy its ID
// 2. Go to https://script.google.com → New project
// 3. Paste this code (replace YOUR_SHEET_ID)
// 4. Run "setupSheets" once (click ▶ Run)
...
// ───��� ROLES ────
// owner = can see everything, manage users
// ea = can see owner's calendar + owner's tasks, can complete on behalf of owner
// member = can see only their own calendar + tasks assigned to/by them
...
function setupSheets() {
  ...
  if (!tSheet) {
    ...
    tSheet.appendRow(['id', 'title', 'description', 'assignedTo', 'assignedBy', 'status', 'priority', 'dueDate', 'createdBy', 'createdAt', 'updatedAt']);
    tSheet.getRange('1:1').setFontWeight('bold').setBackground('#F43F5E').setFontColor('#FFFFFF');
    tSheet.setFrozenRows(1);
  }

  return 'Setup complete! Users, Events, and Tasks sheets created. Default owner: Jitendra / admin123';
}
...
function doGet(e) {
  ...
  try {
    ...
    switch (action) {
      case 'ping': result = { status: 'ok', version: '3.0' }; break;
...
function doPost(e) {
  ...
  try {
    ...
    // Verify user exists for all POST actions
    if (!verifyUser(user)) {
...
function getUsers(requestedBy) {
  ...
  // Only owner can see all users with full details
  const ss = SpreadsheetApp.openById(SHEET_ID);
  const sheet = ss.getSheetByName(SHEETS.USERS);
  const data = sheet.getDataRange().getValues();
  const users = [];
...
function getTasks(user) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!row[0]) continue;
    const task = {};
    headers.forEach((h, j) => { task[h] = row[j]; });
    ...
    if (role === ROLES.OWNER) {
      tasks[row[0]] = task; // Owner sees everything
    } else if (role === ROLES.EA) {
      // EA sees: ALL tasks assigned to owner + tasks assigned to/by EA
      if (assignedTo === ownerName || assignedTo === userLower || assignedBy === userLower || createdBy === userLower) {
        tasks[row[0]] = task;
      }
    } else {
      // Member: assigned to them OR assigned by them OR created by them
...
function saveTask(task, user) {
  ...
  // EA assigns tasks on behalf of owner
  ...
  const rowData = headers.map(h => task[h] || '');
  if (rowIndex > 0) {
    sheet.getRange(rowIndex, 1, 1, headers.length).setValues([rowData]);
  } else {
    sheet.appendRow(rowData);
...
function updateTaskStatus(id, status, user) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === id) {
      // Check permission: owner can update any, EA can update owner's tasks + own, member can update own
